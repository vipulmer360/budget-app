...
const Reports = {
  ...
  _daybookReport() {
    ...
    const transactions = [
      ...incomes.map(i => ({ date: i.date, type: 'Income', account: i.accountName, credit: Calculations.getItemAmount(i), debit: 0, notes: i.notes })),
      ...expenses.map(e => ({ date: e.date, type: 'Expense', account: e.accountName, credit: 0, debit: Calculations.getItemAmount(e), notes: e.notes }))
    ...
    return `
      ...
      <div class="table-container">
        <table class="data-table">
          <thead><tr><th>Date</th><th>Type</th><th>Item Name</th><th>Account</th><th>Notes</th><th class="text-right">Credit</th><th class="text-right">Debit</th></tr></thead>
          <tbody>
            ${transactions.map(t => `
              <tr>
                <td>${Utils.formatDate(t.date)}</td>
                <td><span class="badge ${t.type === 'Income' ? 'badge-success' : 'badge-danger'}">${t.type}</span></td>
                <td>${Utils.escapeHtml(t.account || 'Cash')}</td>
  ...
  exportCSV() {
    ...
    let csv = 'Date,Type,Account,Amount,Notes\n';
    incomes.forEach(i => {
      csv += `${i.date},Income,"${i.accountName}",${i.amount},"${i.notes || ''}"\n`;
    });
    expenses.forEach(e => {
      csv += `${e.date},Expense,"${e.accountName}",${e.amount},"${e.notes || ''}"\n`;
    });
